...
- **`slicer-mill/` holds only plugin-shaped suites** — test whose subject is emission belongs in `slicer-core/`, whatever build tool it drives.
- **Emitted-build suites are twins**, one per build tool, and the only place slice is compiled through its own generated build. Keep them symmetric: same root, same assertions, only file names and syntax differ. The mill twin must assert class files came out, never just exit code.
