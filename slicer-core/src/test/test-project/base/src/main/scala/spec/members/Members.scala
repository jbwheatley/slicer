...
object OverloadedMembers {
  ...
  def withDefaultAndVarargs(separator: String = ", ", parts: String*): String = parts.mkString(separator)

  def withByNameParameter(compute: => String): String = compute + compute

  def callsAll(): String =
    overloaded(1) + overloaded("a") + overloaded("b", 2) + withDefaultAndVarargs("-", "x", "y") +
